    ...
    if (currentIndex === -1) {
      ...
      if (items && items.length > 0) {
        const maxCodeNum = items.reduce((max, item) => {
          const codeToCheck = item.lensColourCode;
          if (codeToCheck && codeToCheck.startsWith("LC")) {
        ...
        nextCode = "LC" + nextNum.toString().padStart(3, "0");
      }
      
      if (formData.lensColourCode !== nextCode) {
        setFormData(prev => ({ ...prev, lensColourCode: nextCode }));
      }
    }
  }, [items, currentIndex, formData.lensColourCode, setFormData]);
  ...
  return (
    <div className="master-container">
      <div className="master-header">
        <h2>
          <FaGlasses /> Lens Colour Management
        </h2>
